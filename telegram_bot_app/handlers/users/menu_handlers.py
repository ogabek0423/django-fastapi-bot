...
async def list_categories(message: Union[CallbackQuery, Message], **kwargs):
    ...
    # Agar foydalanuvchidan Callback kelsa Callback natijasini o'zgartiramiz
    elif isinstance(message, CallbackQuery):
        call = message
        await call.message.edit_reply_markup(markup)

# Ost-kategoriyaga tegishli mahsulotlar ro'yxatini yuboruvchi funksiya
async def list_items(callback: CallbackQuery, category_id, **kwargs):
...
async def show_item(callback: CallbackQuery, category_id, item_id):
    markup = item_keyboard(category_id, item_id)
...
async def navigate(call: CallbackQuery, callback_data: dict):
    ...
    # Convert item_id to integer if it is not None
    if item_id:
        item_id = int(item_id)
    ...
    await current_level_function(
        call, category_id=int(category), item_id=item_id
    )